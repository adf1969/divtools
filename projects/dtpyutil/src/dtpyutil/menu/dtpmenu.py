...
class DtpMenuApp(App):
    ...
    BINDINGS = [
        ...
        Binding("0", "jump_to_item('0')", show=False),
        Binding("1", "jump_to_item('1')", show=False),
        Binding("2", "jump_to_item('2')", show=False),
        Binding("3", "jump_to_item('3')", show=False),
        Binding("4", "jump_to_item('4')", show=False),
        Binding("5", "jump_to_item('5')", show=False),
        Binding("6", "jump_to_item('6')", show=False),
        Binding("7", "jump_to_item('7')", show=False),
        Binding("8", "jump_to_item('8')", show=False),
        Binding("9", "jump_to_item('9')", show=False),
    ]
    ...
    def __init__(self, mode, title, content_data, width, height, colors, h_center=False, v_center=False, debug=False, output_file=None):
        ...
        # Dynamic CSS override for centering
        # Apply centering based on flags, with proper fallback
        if h_center or v_center:
            align_spec = "center " if h_center else ""
            align_spec += "middle" if v_center else "auto"
            self.CSS_OVERRIDE = f"""
            Screen {{
                align: {align_spec.strip()};
            }}
            """
            if self.debug_mode:
                print(f"[DEBUG] Applied CSS centering override: align: {align_spec.strip()}", file=sys.stderr)
    ...
    def on_mount(self) -> None:
        # Apply centering if flags are set
        if self.h_center or self.v_center:
            align_h = "center" if self.h_center else "left"
            align_v = "middle" if self.v_center else "top"
            self.screen.styles.align = (align_h, align_v)
            if self.debug_mode:
                print(f"[DEBUG] Applied screen alignment: {align_h} {align_v}", file=sys.stderr)
    ...
    def _compose_menu(self):
        ...
        if self.content_data:
            for tag, text in self.content_data:
                if tag:  # Regular menu item with tag
                    # Use the provided tag (e.g., "1", "2",..., "0") instead of auto-numbering
                    display_text = f"({tag}) {text}"
                    items.append(ListItem(Label(display_text), id=f"item-{tag}"))
                else:  # Section header (empty tag) - display as separator
                    # Render as a ListItem but with special styling to indicate it's a header
                    header_display = f"  {text}"
                    items.append(ListItem(Label(header_display, classes="section-header"), id=f"header-{len(items)}"))
    ...
    def _compose_msgbox(self):
        ...
        if isinstance(self.content_data, dict):
            ...
            align = self.content_data.get("align", "center")
        else:
            msg = self.content_data if self.content_data else ""

        classes = "message-text-left" if align == "left" else "message-text"
        yield Static(msg, classes=classes)
        yield Horizontal(
    ...
    def _compose_inputbox(self):
        ...
        yield Static(msg, classes="message-text")
        input_field = Input(value=default, id="input-field")
        yield input_field
        yield Horizontal(
            Button("OK", variant="primary", id="btn-ok"),
            Button("Cancel", variant="error", id="btn-cancel"),
        ...
        # Set focus on input field initially
        input_field.focus()

    def on_list_view_selected(self, event: ListView.Selected):
        item_id = str(event.item.id)
        # Skip section headers (they have IDs starting with "header-")
        if item_id.startswith("header-"):
            return
        self.result = item_id.replace("item-", "")
        self.exit_code = 0
    ...
    def action_jump_to_item(self, tag: str):
        """Jump to menu item with specified tag (number) - for numeric shortcuts"""
        ...
        try:
            ...
            for i, child in enumerate(list_view.children):
                if child.id == item_id:
                    # Focus and highlight this item
                    list_view.index = i
                    if self.debug_mode:
                        print(f"[DEBUG] Jumped to menu item with tag '{tag}'", file=sys.stderr)
                    return
            
            # Item not found - log if debug
            if self.debug_mode:
                print(f"[DEBUG] Menu item with tag '{tag}' not found", file=sys.stderr)
        except Exception as e:
            if self.debug_mode:
                print(f"[DEBUG] Error jumping to item: {e}", file=sys.stderr)
