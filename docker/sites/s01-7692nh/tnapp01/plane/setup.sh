...
function download() {
    ...
    # CUSTOM MODIFICATION: Skip docker-compose.yaml download if it exists
    if [ -f "$PLANE_INSTALL_DIR/docker-compose.yaml" ]
    then
        echo "*** SKIPPING docker-compose.yaml download - file exists and has custom modifications ***"
        echo "*** If you need to update it, manually delete or rename the file first ***"
    else
        # Original download logic for docker-compose.yaml
    ...
    # CUSTOM MODIFICATION: Skip plane.env overwrite if it exists
    if [ -f "$DOCKER_ENV_PATH" ];
    then
        echo "*** SKIPPING plane.env overwrite - file exists and has custom modifications ***"
        echo "*** A backup copy is saved to $PLANE_INSTALL_DIR/archive/$TS.env ***"
        echo "*** New variables template saved to $PLANE_INSTALL_DIR/variables-upgrade.env for reference ***"
        cp "$DOCKER_ENV_PATH" "$PLANE_INSTALL_DIR/archive/$TS.env"
        # Keep the existing plane.env, don't overwrite it
    else
        # Original logic - only if plane.env doesn't exist
